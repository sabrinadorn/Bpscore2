* This is the file of Becker and Ichino as delivered with their STATA package 
* (SJ2-4: st0026
* VERSION 1.29      (SJ2-4: st0026)
* OCTOBER 8, 2002
* LAST REVISOR: SOB
*
*
...
syntax varlist [if] [in], pscore(string) blockid(string) [ comsup DETail BOOTstrap Reps(int 50) NOIsily Dots ]
tokenize `varlist'
...
bs "atts `varlist' `if' `in', pscore(`pscore') blockid(`blockid') `comsup' " "r(atts)", nowarn reps(`reps') `noisily' `dots'

}

if `"`bootstrap'"' != `""'  {  

return scalar atts = r(stat)
return scalar bseatts = r(se)
